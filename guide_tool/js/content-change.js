...
function show_prepare(step){
	...
	if (step>=5 && flag_check==1){
		...
		for (var j=0;j<solenoids_each_mode.length;j++){
			...
			new_tr.innerHTML = "<td> <p id='situation-"+j+"' style='font-size:12px;'>"+mode_a[j]+"</p> </td>  <td> <p id='open-"+j+"' style='font-size:12px;'> "+solenoids_each_mode[j]+
			"</p> </td> <td> <p id='close-"+j+"' style='font-size:12px;'> "+a+"</p> </td>"
		...
		for (var i=1; i<=c.length; i++){
			var new_tr = document.createElement("tr");
			new_tr.innerHTML = "<td> <p id='solenoid-id-"+i+"' style='font-size:12px;'>"+"solenoid"+i+"</p> </td> <td> <p id='port5-2-"+i+"' style='font-size:12px;'> "+c[i-1]+ "</p> </td>"
			document.getElementById("tb5-2").appendChild(new_tr);
		}
		for (var i=1; i<=c.length; i++){
			var new_tr = document.createElement("tr");
			new_tr.innerHTML = "<td> <p id='solenoid-sample-"+i+"' style='font-size:12px;'>"+"solenoid"+i+"</p> </td> <td> <p id='port5-3-"+i+"' style='font-size:12px;'> "+switch_s[i-1]+ "</p> </td>"
...
function show(i){
	...
	if (step==4){
		...
		document.getElementById("inletshow").innerHTML = '['+ inlet + ']';
		var port_flow = Array.from({length: component_exist_num[0]}, (_, i) => i + 1)
		outlet = port_flow.filter( ( el ) => !inlet.includes( el ) );
		document.getElementById("outletshow").innerHTML = '['+ outlet + ']';
	}
	if (step ==1){
...
function add_row(){
	...
	obj.innerHTML = "<td> <h1 id='num_id'"+l+">"+point_path[l-1]+"</h1> </td> <td> <input type='text' class='input' id='media'"+l+"> </td> <td> <button class='mode' id='remove4-"+l+
